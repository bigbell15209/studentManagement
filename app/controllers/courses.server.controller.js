...
exports.list = function (req, res) {
    Course.find().sort('-created').populate('applicants', 'firstName lastName fullName').exec((err, courses) => {
if (err) {
        return res.status(400).send({
            message: getErrorMessage(err)
        });
...
exports.courseByID = function (req, res, next, id) {
    Course.findById(id).populate('applicants', 'firstName lastName fullName').exec((err, course) => {if (err) return next(err);
...
exports.hasAuthorization = function (req, res, next) {
    console.log('in hasAuthorization - applicants: ',req.course.applicants)
